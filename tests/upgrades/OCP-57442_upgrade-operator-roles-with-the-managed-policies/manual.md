...
Wait the cluster ready then upgrade the operator roles
\# rosa upgrade operator-roles -c <>
\# rosa upgrade operator-roles -c <> --version <upgrading cluster version>

## Expect

- If there is no additional role or no '--version' specified, noting will be done and some message shown:
  I: Cluster 'yuwan-ists1' operator roles have attached managed policies
...
For HCP cluster below message will be shown
zhewang@fedora:~$ rosa upgrade operator-roles -c zwant9 -m manual
...
For HCP cluster below message will be shown
zhewang@fedora:~$ rosa upgrade operator-roles -c zwant9 -m auto
I: Cluster 'zwant9' operator roles have attached managed policies. An upgrade isn't needed
