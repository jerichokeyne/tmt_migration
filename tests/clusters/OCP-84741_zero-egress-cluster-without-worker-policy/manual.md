...
3. remove worker policy from worker role:
...
5. Create an HCP cluster with zero-egress properties enabled (version 4.14.35 only, zero_egress set to true and the specific provision shard)
...
6. Once cluster is created, verify that the cluster is set to zero_egress true
...
7. Create a new machinepool on the cluster
...
8. List machinepools associated with the cluster
...
9. Update the machinepool to enable autorepair, and 5/20 replicas
