...
## Step
Edit the cluster interactive mode then choose both Yes at the questions of '--private' and '--disable-workload-monitoring', then check the `rosa describe cluster` output

## Expect
- The result of 'Private' is changed from Yes to No and the api response api.listening=external  
-~~The result of 'User Workload Monitoring' is set with 'disabled' ~~- Since OCM17719, there is no qeustionair for disable-workload-monitoring, that's deprecated

## Step
Edit the cluster interactive mode then choose both No at the questions of '--private' and '--disable-workload-monitoring', then check the `rosa describe cluster` output
...
- Since OCM17719, there is no qeustionair for disable-workload-monitoring, that's deprecated
