...
```
Create cluster-specific Operator Identity and Access Management (IAM) roles based on your cluster configuration.
...
- The usage should be clear and correct
```
Create OpenID Connect (OIDC) provider for operators to authenticate against in an AWS Security Token Service (STS) cluster.
