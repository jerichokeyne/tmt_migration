...
## Step
Create hosted-cp operator-roles with `--vpc-endpoint-role-arn` and `--route53-role-arn` in manual mode.
...
## Expect
- There are AWS commands prompted. There are two additional commands to create assume-role policies, with the `sts:AssumeRole` permission on the route53-role and vpc-endpoint-role.
...
## Expect
- The assume role policies path should be same with the `vpc-endpoint-role-arn` and `route53-role-arn` path settings.
...
## Expect
All roles are deleted successfully by the prompted commands. There are commands to delete two created assume role policies.
...
- It will returns error when execute the commands. Actually the error is from the AWS CLI.
