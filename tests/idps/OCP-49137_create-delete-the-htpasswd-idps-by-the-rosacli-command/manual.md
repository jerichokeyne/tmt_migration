...
## Step
Create HTPasswd IDP via rosacli command  
\#rosa create idp --name cidph --type htpasswd -c 1quu0ukmeffe30h0dvslrkhf9g60hhek --username chtpidpwy1 --password chtpi#qs$1

## Expect
- The idp with the user is created and the cluster console can be logged with the username and password  
...
## Step
Create HTPasswd idp with --users  
- Set one set of user  
- set multi sets of users  
...
## Step
Create HTPasswd idp with --from-file  
...
NOTE: the htpasswd file can be generate with htpasswd command
...
- Set two or three of --users and --username and --password and --from-file at same time

## Expect
1.2. There should be validation for them.  
3. It should fail with error/warning message   
