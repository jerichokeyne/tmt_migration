...
## Step
Create an HTPasswd IDP in interactive mode.
...
## Expect
- It allows creating multiple users in interactive mode.
- The help message should be shown after entering `?`.
- Querying the IDP through the OCM API should show the result correctly.
...
- There is an option, `? Configure users from HTPasswd file (optional):`, to input the path of the htpasswd IDP file. It will continue to ask `? Add another user` after adding by file.
- If the file cannot be found at the path, it returns an error. If the username exists in a file, it returns an error.
