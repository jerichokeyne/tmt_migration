...
```
Create OpenID Connect (OIDC) config in an S3 bucket for the client AWS account and populates it to be compliant with OIDC protocol. It also creates a Secret in Secrets Manager containing the private key.
...
## Expect
- There are three files using for creating oidc config generated under the current folder: `rosa-private-key-oidc-b4k8.key`, `discovery-document-oidc-w5p2.json`, and `jwks-oidc-w5p2.json`.
