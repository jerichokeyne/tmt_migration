...
```bash
rosa verify permissions --help
rosa verify openshift-client --help
rosa verify quota --help
```

```
Verify AWS permissions needed to create a non-AWS Security Token Service (STS) cluster are configured as expected
...
## Step
Configure AWS with all permissions and quota, and have `openshift-client` in the local environment. Then verify them.
...
## Step
Try to verify permissions, quota, and `openshift-client` when they are missing.

## Expect
There should be an error message.

## Step
Check the ROSA CLI version with `rosa version`.
...
- The output should contain the Git SHA from OCM-5782.

## Step
Repeat the steps on Windows, macOS, and Linux.
